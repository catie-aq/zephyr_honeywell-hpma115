...
static void hpma115_write_command(const struct device *dev, uint8_t cmd_length)
{
	...
	LOG_DBG("Tx buf: %X %X %X %X", data->tx_buf[0], data->tx_buf[1], data->tx_buf[2],
		data->tx_buf[3]);
...
#define HPMA115_INIT(inst)                                                                         \
	...
	static const struct hpma115_conf hpma115_conf_##inst = {                                   \
		...
		.cb = hpma115_uart_isr,                                                            \
                                                                                                   ...
                                                                                                   \
	SENSOR_DEVICE_DT_INST_DEFINE(inst, hpma115_init, NULL, &hpma115_sensor_data_##inst,        \
				     &hpma115_conf_##inst, POST_KERNEL,                            \
				     CONFIG_SENSOR_INIT_PRIORITY, &hpma115_api_funcs);
